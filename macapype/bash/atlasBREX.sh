...
Optional arguments:
    ...
    -g          vertical gradient in fractional intensity threshold (-1->1); default=0; 
                ...
                used for bet initialization only
...
do
  case "$1" in
    ...
    ;;
    -g |-G )
    if [[ -n "$2" ]]; then vthr_="$2"; else echo $SYN; exit 1 ; fi
    shift 2
    ;;
    -help|--help)
    HELP >&2
...
#prep region
    ...
    #set flag for vertical bet gradient
    vthr=0
    if [[ -n "${vthr_}" ]]
    then
        if [[ $(echo "(${vthr_} > -1) && (${vthr_} < 1)" | bc -l) -eq 1 ]]
        then
        vthr=${vthr_}
        else
            echo '>> Vertical threshold must be needs to be a number greater than -1 and less than 1. See -help'
            exit 1
        fi
    fi
...
#functions region
    ...
    frct=$3
        ...
        #BET binary mask 
            if [[ $frct == "n" ]]
            then
                $FSLDIR/bin/bet $filename ${filename%%.*}temp_option_1 -f 0.3 -R -g $vthr -m | tee -a log.txt
                wait
                $FSLDIR/bin/bet $filename ${filename%%.*}temp_option_2 -f 0.5 -R -g $vthr -m | tee -a log.txt
                wait
                $FSLDIR/bin/bet $filename ${filename%%.*}temp_option_3 -f 0.8 -R -g $vthr -m | tee -a log.txt
                wait
                ...
                read -p "Option: " option
                    ...
                    then
		                ...
		                if [[ $option_ =~ ^([0-9]*[1-9][0-9]*(\.[0-9]+)?|[0]*\.[0-9]*[1-9][0-9]*)$ ]]
		               	then
	                	$FSLDIR/bin/bet $filename ${filename%%.*}temp_1 -f $option_ -R -g $vthr -m | tee -a log.txt
	                	wait
                	...
                	$FSLDIR/bin/bet $filename ${filename%%.*}temp_1 -f $frct -R -g $vthr -m | tee -a log.txt
                	wait
                	mask=${filename%%.*}temp_1_mask.nii.gz
        	fi
...
#highres_linear
    ...
    case $METHOD in
       ...
       #FSL
            ...
            echo ${FSLDIR}/bin/flirt -ref $highres_BE -in $standard_BE -omat std2high.mat | tee -a log.txt
